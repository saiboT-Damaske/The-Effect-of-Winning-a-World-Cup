...
\midrule
2022 & Argentina      & Qatar          & Q4 & \multicolumn{2}{c}{\textit{Potential case study}} \\
\bottomrule
\end{tabular}
\begin{tablenotes}
\small
...
Table~\ref{tab:summary_comparison} presents the core validation step: a side-by-side comparison of the summary statistics from \citet[Table~1]{Mello2024} (reproduced in the left panel) and the corresponding statistics from this replication (right panel). The table follows the same format as the original, reporting GDP, population, GDP per capita, and year-over-year GDP growth by sub-period and winner status.
