# ============================================================
# Finalist Event Study — GDP only (log YoY)
...
# Output: finalist_analysis/results/finalist_event_study_coefficients.csv
#         finalist_analysis/plots/finalist_event_study_gdp.png
# ============================================================
...
# ---------- 2) Identify finalist events (reached WC final) ----------
# rank1 = winner, rank2 = runner-up  →  finalist = rank1 | rank2
finalist_events <- df0 %>%
...
cat("\nFinalist countries:", paste(finalist_countries, collapse = ", "), "\n")

cat("\nFinalist events:\n")
print(as.data.frame(finalist_events))

# ---------- 3) Prepare panel ----------
df <- df0 %>%
  mutate(
    is_finalist = as.integer(country %in% finalist_countries),
    host        = as.integer(host)
  ) %>%
  rename(dy_gdp = gdp_yoy_log_4q)

# ---------- 4) Assign relative time to nearest finalist event ----------
...
df <- df %>%
  ...
  group_modify(~ {
    ctry <- .y$country[[1]]
    ev   <- finalist_events %>% filter(country == ctry) %>% pull(tq_event)
    if (length(ev) > 0) {
...
cat("  Finalists:   ", sum(df_cc$is_finalist == 1), "obs from",
    n_distinct(df_cc$country[df_cc$is_finalist == 1]), "countries\n")

# ---------- 6) Estimate with fixest ----------
m <- feols(
  dy_gdp ~ i(rel_time_bin, is_finalist, ref = 0) + host + ln_gdp_l4
         | country + tq,
  cluster = ~country,
  data    = df_cc
)
...
cat("\n--- Finalist event study coefficients ---\n")
print(as_tibble(tab), n = Inf)

# ---------- 8) Save results ----------
write_csv(tab, "finalist_analysis/results/finalist_event_study_coefficients.csv")
cat("\nSaved: finalist_analysis/results/finalist_event_study_coefficients.csv\n")
...
ggsave("finalist_analysis/plots/finalist_event_study_gdp.png",
       gg, width = 7, height = 4, dpi = 300)
ggsave("finalist_analysis/plots/finalist_event_study_gdp.pdf",
       gg, width = 7, height = 4, dpi = 300)
cat("Saved: finalist_analysis/plots/finalist_event_study_gdp.{png,pdf}\n")

# ---------- 10) Key summary statistics ----------
cat("\n========================================\n")
cat("  FINALIST EVENT STUDY SUMMARY (GDP)\n")
cat("========================================\n")
cat("  N countries treated:", length(finalist_countries), "\n")
cat("  N finalist events: ", nrow(finalist_events), "\n")
