# ============================================================
# SDiD: Semi-Finalist Treatment — GDP only
# Treatment: reaching the World Cup Top 4 (rank1 OR rank2 OR rank3 OR rank4)
# Same stacked panel design as paper_replicate_sdid_full_features.R
# but with semi-finalist treatment and GDP only.
#
# Input:  Data/mello_paper_replication/paper_replication_sample.csv
# Output: semi_finalist_analysis/results/semi_finalist_sdid_gdp_results.csv
#         semi_finalist_analysis/plots/semi_finalist_sdid_gdp.png
# ============================================================
...
# ---------- 3) Semi-finalist treatment: reached WC top 4 in that year ----------
# Build lookup from the data itself (rank1 or rank2 or rank3 or rank4 at Q2)
semi_finalist_lookup <- df %>%
...
cat("\nSemi-finalist events in data:\n")
print(as.data.frame(semi_finalist_lookup))

is_semi_finalist_subseries <- function(country, wc_year) {
  any(semi_finalist_lookup$country == country &
      semi_finalist_lookup$wc_year == wc_year)
}
...
cat("\n================================================\n")
cat("SDiD: Semi-Finalist treatment — GDP (log YoY)\n")
cat("================================================\n")
...
if (inherits(p_obj, "ggplot")) {
  p_final <- p_obj +
    ggplot2::ggtitle("SDiD: Semi-Finalist effect on YoY GDP growth (pp)") +
  ...
  ggsave("semi_finalist_analysis/plots/semi_finalist_sdid_gdp.png",
         p_final, width = 10, height = 6, dpi = 300)
  cat("Saved: semi_finalist_analysis/plots/semi_finalist_sdid_gdp.png\n")
} else {
  png("semi_finalist_analysis/plots/semi_finalist_sdid_gdp.png",
      width = 10, height = 6, units = "in", res = 300)
  plot(tau_hat)
  graphics::title(main = "SDiD: Semi-Finalist effect on YoY GDP growth (pp)",
                  sub = subtitle_text)
  dev.off()
  cat("Saved: semi_finalist_analysis/plots/semi_finalist_sdid_gdp.png\n")
}
...
write_csv(results_df, "semi_finalist_analysis/results/semi_finalist_sdid_gdp_results.csv")
cat("Saved: semi_finalist_analysis/results/semi_finalist_sdid_gdp_results.csv\n")
